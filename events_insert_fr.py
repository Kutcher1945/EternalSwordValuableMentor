...
async def insert_or_update_events_data(event_data, conn):
    try:
        ...
        event_attributes = event_data["event"].get("attributes")
        if event_attributes is None:
            ...
            age = event_attributes.get("age")
            mask = event_attributes.get("mask")
            race = event_attributes.get("race")
            beard = event_attributes.get("beard")
            gender = event_attributes.get("gender")
            glasses = event_attributes.get("glasses")
            race_score = event_attributes.get("raceScore")
            beard_score = event_attributes.get("beardScore")
            gender_score = event_attributes.get("genderScore")
            glasses_score = event_attributes.get("glassesScore")

        # Check if the event exists
        cursor.execute("SELECT COUNT(*) FROM esvm_events_fr WHERE event_id = %s", (event_data["event_id"],))
        count = cursor.fetchone()[0]
        if count > 0:
            # Update the existing event
            update_event_query = """
                UPDATE esvm_events_fr
                SET camera_id = %s, camera_name = %s, place_id = %s, place_name = %s,
                    ...
                    age = %s, mask = %s, race = %s, beard = %s, gender = %s, glasses = %s,
                    race_score = %s, beard_score = %s, gender_score = %s, glasses_score = %s
                WHERE event_id = %s
            """
            cursor.execute(update_event_query, (
                ...
                event_data["obsolete"],
                age, mask, race, beard, gender, glasses, race_score, beard_score, gender_score, glasses_score,
                event_data["event_id"]
            ))
        else:
            # Insert the new event
            insert_event_query = """
                INSERT INTO esvm_events_fr (
