...
async def insert_or_update_events_data(event_data, conn):
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM esvm_events_ao WHERE event_id = %s", (event_data["event_id"],))
        count = cursor.fetchone()[0]
        if count > 0:
            # Update the existing event
            update_event_query = """
                UPDATE esvm_events_ao
            ...
            cursor.execute(update_event_query, (
                ...
                json.dumps(event_data["event"]["attributes"]), json.dumps(event_data["event"]["roi"]),
                event_data["snapshot_url"], event_data["streamserver_id"], event_data["stream_id"],
                event_data["obsolete"], event_data["event_id"]
            ))
        else:
            # Insert the new event
            insert_event_query = """
                INSERT INTO esvm_events_ao (
