...
async def get_events_data_with_pagination(url, token, ts_from, ts_to, event_type, cursor=None, limit=20):
    ...
    while retries > 0:
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to get events data. Status code: {response.status_code}. Retrying...")
            retries -= 1
            delay *= 2  # Exponential backoff
            await asyncio.sleep(delay)
    print("Failed to get events data after retries.")
    return None
...
async def insert_or_update_events_data(event_data, conn):
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM esvm_events_tp WHERE event_id = %s", (event_data["event_id"],))
        count = cursor.fetchone()[0]
        if count > 0:
            # Update the existing event
            update_event_query = """
                UPDATE esvm_events_tp
                SET camera_id = %s, camera_name = %s, place_id = %s, place_name = %s,
                    ...
                    event_detection = %s, event_attributes = %s
                WHERE event_id = %s
            """
            cursor.execute(update_event_query, (
                ...
                json.dumps(event_data["event"]["attributes"]), event_data["event_id"]
            ))
        else:
            # Insert the new event
            insert_event_query = """
                INSERT INTO esvm_events_tp (
            ...
            cursor.execute(insert_event_query, (
                ...
                datetime.fromtimestamp(event_data["ts"] / 1000), event_data["event"]["reason"],
                event_data["event"]["label"], json.dumps(event_data["event"]["detection"]),
                json.dumps(event_data["event"]["attributes"])
            ))
        conn.commit()
