...
async def insert_or_update_events_data(event_data, conn):
    try:
        ...
        if event_attributes is None:
            ...
            make = event_attributes.get("make")
            event_type = event_attributes.get("type")
            color = event_attributes.get("color")
            model = event_attributes.get("model")

        # Check if the event exists
        cursor.execute("SELECT COUNT(*) FROM esvm_events_lpr WHERE event_id = %s", (event_data["event_id"],))
        count = cursor.fetchone()[0]
        if count > 0:
            # Update the existing event
            update_event_query = """
                UPDATE esvm_events_lpr
                SET camera_id = %s, camera_name = %s, place_id = %s, place_name = %s,
                    ...
                    make_id = %s, type = %s, color_id = %s, model_id = %s
                WHERE event_id = %s
            """
            cursor.execute(update_event_query, (
                ...
                event_data["obsolete"],
                make, event_type, color, model,
                event_data["event_id"]
            ))
        else:
            # Insert the new event
            insert_event_query = """
                INSERT INTO esvm_events_lpr (
                    ...
                    make_id, type, color_id, model_id
                ) VALUES (
                    %s, %s, %s, %s, %s,
                    %s, %s, %s, %s, %s,
                    %s, %s, %s, %s, %s,
